...
//   2. the two out-of-range verdicts, which must be reported ON the calculator
//      and must not fly the operator anywhere
...
async function page(vp) {
	...
	p.on('console', (m) => {
		if (m.type() === 'error') p.__errs.push(m.text());
	});
	return p;
}
...
const runToRoom = async (p, max = 200) => {
	...
	for (let i = 0; i < max; i++) {
		if (
			await p.evaluate(
				() => !!document.querySelector('.again') || !!document.querySelector('.card')
			)
		)
			return true;
...
for (const [name, vp] of [
	...
	const fit = await p.evaluate(() => {
		...
		const go = q('.go'),
			...
			win = q('.window');
		return {
			overY: Math.round(go.bottom - innerHeight),
			overX: Math.round(ctl.right - innerWidth),
			plateOverlapsWin: plate.bottom > win.top,
			gap: Math.round(win.top - plate.bottom)
		};
	});
...
// ── 2. Out-of-range verdicts stay on the home screen ─────────────────────
for (const [name, m, d, y] of [
	['past', '1', '1', '1958'],
	['future', '12', '31', String(new Date().getFullYear())]
]) {
	...
	const v = await p.evaluate(() => {
		const c = document.querySelector('.calculator');
		return {
			stillHome: !!c && !!document.querySelector('.controls'),
			verdict: document.querySelector('.verdict .msg')?.textContent?.trim().slice(0, 40) ?? null,
			// it must NOT have been pushed into the lens
			tf: c ? getComputedStyle(c).transform : 'gone',
			flew: !!document.querySelector('.glass') || !!document.querySelector('.again')
		};
	});
	ok(`edge/${name}: stays on the calculator`, v.stillHome, v.tf);
	ok(`edge/${name}: verdict shown on the machine's screen`, !!v.verdict, v.verdict);
	ok(`edge/${name}: did not fly anywhere`, !v.flew && v.tf === 'none');
	// and changing the dials clears it, so the operator can just go again
	await p.selectOption('select[aria-label=year]', '1986');
	await p.waitForTimeout(400);
	ok(
		`edge/${name}: clears when the dials change`,
		await p.evaluate(() => !document.querySelector('.verdict'))
	);
	ok(`edge/${name}: a real date then runs`, await runToRoom(p));
...
// ── 3. The loop: return moves the scene, answers kept, second run works ────
{
	...
	ok(
		'loop: transform settles to identity',
		kept.tf === 'none' || kept.tf === 'matrix(1, 0, 0, 1, 0, 0)',
		kept.tf
	);
...
}
{
	...
	let landed = false;
	for (let i = 0; i < 200; i++) {
		if (await p.evaluate(() => !!document.querySelector('.again'))) {
			landed = true;
			break;
		}
		await p.waitForTimeout(1000);
	...
	ok(
		'robust: result still in the glass after resize',
		await p.evaluate(() => !!document.querySelector('.glass'))
	);
	await p.screenshot({ path: `${OUT}/resize-room.png` });
	ok(
		'robust: no console errors on resize',
		p.__errs.length === 0,
...
// ── 6. Spotify player is reachable (hit-test through the tree) ────────────
{
	...
	const hit = await p.evaluate(() => {
		...
		return {
			ok: el === f,
			tag: el?.tagName,
			cls: (el?.className || '').toString().replace(/s-\w+/g, '').trim(),
