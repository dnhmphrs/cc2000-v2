<script>
	...
	function initializeScene() {
		...
		// Add basic lights
		const light = new THREE.HemisphereLight(0xd0d0d0, 0x0b0b0b, 1.5);
		scene.add(light);
	...
	function updateScene(elapsedTime) {
		...
		if (spermGroup) {
			...
			if ($page === 3) {
				spermGroup.rotation.z = -elapsedTime * 12;
			} else {
				spermGroup.rotation.z = -elapsedTime * 9;
			}
		}
	}
